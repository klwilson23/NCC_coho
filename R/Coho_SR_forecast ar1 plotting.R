...
result_forecast_metrics <- readRDS("Results/coho_forecasting_ar1.rds")
result_forecast_run <- readRDS("Results/coho_forecasting_popdyn_ar1.rds")
result_forecast_RS <- readRDS("Results/coho_forecasting_rec_ar1.rds")
...
resultSR_B3<-readRDS("Results/COSR3B.ar1_lalpha_MCMC.rds")
...
for(i in 1:datamcmc2$Nregs)
{
  ...
  pdf(paste("Figures/coho forecasting ar1 scenario ",i,".pdf",sep=""),height=7,width=10)
...
saveRDS(rmse,file="Results/predictive error ar1.rds")
...
for(i in 1:datamcmc2$Nregs)
{
  ...
  pdf(paste("Figures/coho productivity ar1 forecast scenario ",i,".pdf",sep=""),height=7,width=10)
  layout(matrix(1:12,ncol=4,byrow=TRUE))
  par(mar=c(5,4,0.1,0.1))
  for(j in 1:n.pops)
  {
...
#### result figures:
jpeg(filename="Figures/risk posterior mass results ar1.jpeg", width=8,height=8, units="in", res=600)
mcmc_names <- colnames(result_forecast_run[[1]])
...
#### result figures:
jpeg(filename="Figures/posterior median results ar1.jpeg", width=8,height=8, units="in", res=600)
mcmc_names <- colnames(result_forecast_run[[1]])
prob_fore <- c(0.025,0.4,0.5,0.6,0.975)
