...
data_points <- data_points[data_points$region%in%c("Hecate Lowlands","Inner Waters","Central Coast (South)"),]
...
major_rivers <- dplyr::bind_rows(major_rivers)
...
ncc_inset <- ggdraw(ncc_bec) +
  draw_plot({
    ...
    width = 0.2, 
    height = 0.2)
ggsave('Figures/cc_coho_bec.pdf',plot=ncc_inset,width = 6, height = 7,units='in',dpi=800)
ggsave('Figures/cc_coho_bec.jpeg',plot=ncc_inset,width = 6, height = 7,units='in',dpi=800)
