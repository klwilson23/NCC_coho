...
pop_size <- aggregate(total_run2~population+CU,data=SR.dat,mean,na.rm=TRUE)
pop_size <- aggregate(total_run2~CU,data=pop_size[pop_size$CU%in%c("bella_coola_dean","hecate_strait_mainland","northern_coastal","rivers_inlet","douglas_channel_kitimat"),],sum,na.rm=TRUE)
sum(pop_size$total_run2)
...
resultSR_B3<-readRDS("Data/COSR3B.cormat_lalpha_MCMC.rds")
...
variables_1 <- c("msy_ratio","run_ratio")#,"beta","tau.alpha","sigma.alpha","tau","sigma") # these are the variables to keep track of
...
temp3B <- jags.model(file="JAGS/forecasting model.jags", data=datamcmc2 ,n.chains = n_chains, n.adapt=50000, quiet=FALSE,inits=init_fx)#, inits=inits)
...
saveRDS(result_forecast_metrics,file="Results/coho_forecasting.rds")
saveRDS(result_forecast_run,file="Results/coho_forecasting_popdyn.rds")
saveRDS(result_forecast_RS,file="Results/coho_forecasting_rec.rds")

saveRDS(datamcmc2,file="Data/jags_forecasting_data.rds")
